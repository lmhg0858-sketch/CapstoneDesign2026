...
class VisionService:
    ...
    def analyze_single_crop(
        self, cropped_base64: str, food_list: list = None
    ) -> tuple:
        """🎯 [고도화 패치]: 크롭한 이미지 조각을 기반으로 음식명과 중량(g)을 쌍으로

        추정합니다.
        """
        try:
            allowed_foods = (
                ", ".join(food_list[:400]) if food_list else "한국 음식"
            )
...
"""
            ...
            # -------------------------------------------------------
            # 🛠️ [데이터 정규화 및 오인식 보정 로직 추가]
            # -------------------------------------------------------
            # 요청하신 보정 매핑 규칙 세팅
            correction_map = {
                "샐러드 김밥": "샐러드",
                "샐러드김밥": "샐러드",
                "기스면": "곰탕",  # 기스면이라고 뜨면 곰탕으로 최종 결과 치환
            }

            if food_name in correction_map:
                print(
                    f"⚠️ 오인식 감지 보정 적용: [{food_name}] -> [{correction_map[food_name]}]"
                )
                food_name = correction_map[food_name]
            # -------------------------------------------------------
    ...
    async def analyze_food_image(self, base64_str: str, food_list: list = None):
        ...
        이미지를 받아와 객체 탐지, 식기류 필터링, 크롭 처리 및 GPT 연동을 일괄 수행합니다.
        `main.py`에서 이 함수를 정상적으로 호출하게 됩니다.
        """
        ...
        food_boxes = [
            ...
            for obj in objects
            if obj.name
            not in ["Tableware", "Bowl", "Plate", "Container", "Table ware"]
        ]
        ...
        for obj in food_boxes:
            ...
            # 1. 실제 픽셀 좌표 획득
            xmin, ymin, xmax, ymax = self.get_crop_coordinates(
            ...
            # 4. 개별 크롭 이미지 분석 및 중량 추정 동시 요청 (여기서 치환 로직이 작동합니다)
            food_name, estimated_weight = self.analyze_single_crop(
                cropped_base64, food_list
            )

            # 5. 최종 데이터 포맷 적재
            detected_foods.append({
                "food_name_en": food_name,  # 최종 변환된 명칭이 담깁니다
        ...
        print(
            f"✅ AI 크롭 및 양 추정 최종 완성: {[(f['food_name_en'], f['estimated_weight']) for f in detected_foods]}"
        )
